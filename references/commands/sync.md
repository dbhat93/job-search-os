...
## Architectural Rule: Single Source of Truth for Loop Status

**Status of any interview loop lives in two places only:**
1. **Interview Loops** section: per-company entry with Status field + detail prose
2. **Outcome Log**: terminal events (rejected / advanced / withdrawn / offer)

**Every other surface that displays loop status must be a derived view, regenerated by `sync`, never hand-edited.** Derived views are wrapped in HTML comment markers like:

```
<!-- DERIVED:active_loops START. AUTO-GENERATED by `sync`. DO NOT HAND-EDIT. Last regen: YYYY-MM-DD. -->
...table contents...
<!-- DERIVED:active_loops END -->
```

Current derived blocks (must be regenerated on every full sync):
- `DERIVED:active_loops` (Profile section): table of active loops + recently-closed (last 30 days)
- `DERIVED:priority_stack` (Search Strategy section): ranked active loops with coach note + recently-closed line

**Hand-editing inside DERIVED markers is a bug.** If a status fact appears outside Interview Loops / Outcome Log AND outside a DERIVED block, that's a drift surface and the audit must flag it.

---

## Full Sync Audit

### Step 0: Regenerate Derived Views (always run first)

Before any audit:

1. **Read source of truth**: Interview Loops (current Status per company) + Outcome Log (last 30 days of terminal events).
2. **Find each `<!-- DERIVED:* START -->` block** in `coaching_state.md`.
3. **Rebuild the contents** between START and END markers from source-of-truth data:
   - For `active_loops`: table of all loops with Status != closed/rejected/withdrawn/declined, columns = Company / Stage / Last event / Next action / When. Plus a Recently-closed table (last 30 days).
   - For `priority_stack`: ranked table of active loops. Ranking is coach judgment (decision urgency + coaching leverage); preserve prior ranking unless the audit reveals a reason to change. Coach note per loop is a 1-2 sentence strategic synthesis. Plus a one-line Recently-closed list.
4. **Update the "Last regen" date** in the START marker to today.
5. **Preserve any text outside the markers as-is.**

If a derived block is missing from `coaching_state.md`, create it at its canonical location (Profile for active_loops, Search Strategy for priority_stack).

### Step 0.5: Closed-Loop String Sweep

For every loop with a terminal outcome (rejected / withdrawn / offer-declined / closed) in the **last 30 days**:

1. Grep the entire `coaching_state.md` for the company name.
2. For each hit:
   - **Allowed locations**: Interview Loops section (status should = closed), Outcome Log, DERIVED blocks (regenerated this session, will reflect closure correctly), per-company storybank "loop deployment log" notes.
   - **Drift locations** (flag every hit): Active Coaching Strategy / Current approach prose, 2-week action plan items, Coaching Notes paragraphs, free-text references in Profile or Search Strategy outside DERIVED blocks.
3. For each drift hit, surface the line + propose a rewrite. Do not auto-rewrite prose. Surface and let the candidate confirm.

This is the catch-all that prevents the failure mode where "closed loop is referenced as active" in some prose corner of the file.
