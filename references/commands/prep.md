...
0a. **Phase 0: Positioning Drift Check.** Before any prep work, check Coaching Notes for a Positioning Drift flag (set by COACH.md Sync Drift Check 4). If present AND any of pitch / resume / linkedin has not been updated in >10 sessions, surface to the candidate: "Your [surface] was last updated [N] sessions ago, while your [other] is newer. Interviewers who looked at your LinkedIn or resume will be calibrating to that. Want to reconcile before we prep?" Wait for response. If declined, proceed and note the skipped reconciliation in the session log.
...
3.5. **Archetype Detection.** Run the Archetype Detection Module from `references/modules/archetype-detection-module.md`. Match the parsed JD against the 6 archetype definitions (Platform PM, Founding PM, Domain PM, Regulatory Response PM, Agent/AI PM, GTM/Growth PM). Surface the detected archetype: "Detected archetype: **[type]**. This shapes story prioritization, question prediction, and framing throughout this brief." Save to the Interview Loop entry. If the archetype differs from a previous prep for the same company (e.g., role changed), note the shift.
