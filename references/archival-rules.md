...
## Score History Archival (per-Interview_Type)

When Score History exceeds 15 rows TOTAL, archive per-Interview_Type, not total-row:

1. Group existing rows by `Interview_Type` column (behavioral / live_case / technical_behavioral / system_design / presentation / hybrid / [blank for practice rows]).
2. For each type, keep the most recent 10 rows.
3. Summarize older rows (per type) into the Historical Summary narrative. The summary should preserve per-type: trend direction per dimension, inflection points, and coaching changes that triggered shifts.
4. Practice rows (Interview_Type blank) archive independently using the same 10-row-per-type rule.

**Why per-type**: `progress` velocity analysis compares like-for-like only. If you had 12 behavioral rows and 5 system_design rows (17 total), a pure total-row archival would compress the 7 oldest entries into the narrative regardless of type, potentially thinning system_design to <4 rows and making per-type velocity uncomputable.

Run this check during `progress` or at session start when the file is large.

## Session Log Archival

Apply the same archival pattern to Session Log when it exceeds 15 rows: compress old sessions into a brief narrative, keep recent ones detailed. Session Log has no type-grouping concern; use a straight recent-10 rule. The goal is to keep the file readable and within reasonable context limits for months-long coaching engagements.
