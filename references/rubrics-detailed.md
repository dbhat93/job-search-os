...
| **Interviewer scope mismatch** | Candidate failed to demonstrate dimensions the interviewer never tested; low scores reflect format/scope divergence from prep, not skill gaps. Often visible as: interviewer declined prepped topics, ran a different format than prepped, asked questions outside prepped evaluation axes. Detected via RFV (`cross-cutting.md` → Round Format Verification Module) with verdict Partial or Mismatch. | Any (depends on what the actual round tested) | **Not a candidate drill.** Update loop prep intel so future rounds anticipate the actual format. Downgrade bottleneck diagnosis confidence. Primary Bottleneck does not update from this round. Flag for meta-check log. |
