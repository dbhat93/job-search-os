...
- **`practice`**: Also read `references/role-drills.md`.
- **`mock`**: Firewalled-room architecture. The orchestrator (main session) designs the scenario and briefs a scoped interviewer subagent that gets only the resume, JD, role, and an in-world motive (never the rubric, storybank, or gap list); a separate blind scorer reads the finished transcript with full context. Also read `references/role-drills.md` (personas), `references/cross-cutting.md` (Context-Sensitive Scoring Module, Gap-Handling), `references/commands/prep.md` (format taxonomy, Format Discovery, Technical Format Coaching Boundaries), and `references/challenge-protocol.md` (Level 3 to 5 debrief challenge).
